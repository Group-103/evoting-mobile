...
exports.getBallot = async (req, res) => {
  try {
    ...
    // Get positions filtered by constituency
    const positions = await prisma.position.findMany({
      where: {
        OR: [
          { constituency: null }, // General positions (for everyone)
          { constituency: ballot.voter.constituency } // My constituency positions
        ]
      },
      orderBy: {
        name: 'asc',
  ...
  try {
    ...
    const positions = await prisma.position.findMany({
      where: {
        ...
        // Validate constituency match
        OR: [
          { constituency: null },
          { constituency: ballot.voter.constituency }
        ],
        votingOpens: {
          lte: now,
        },
