      ...
      stream
        ...
        .on('data', async (row) => {
          ...
          try {
            ...
            // Determine constituency based on RegNo range
            // Rule: S24D14/001-005 -> Science, 006-010 -> Arts
            const determineConstituency = (reg) => {
              const parts = reg.split('/');
              if (parts.length < 2) return null;

              const seqStr = parts[parts.length - 1]; // Get last part (e.g., '001')
              const seq = parseInt(seqStr, 10);

              if (isNaN(seq)) return null;

              if (seq >= 1 && seq <= 5) return 'Science';
              if (seq >= 6 && seq <= 10) return 'Arts';

              return null; // General / Unknown
            };

            const constituency = determineConstituency(regNo);
            ...
            if (existing) {
              ...
              await prisma.eligibleVoter.update({
                ...
                data: {
                  ...
                  constituency: constituency, // Auto-assigned
                  status: 'ELIGIBLE',
                },
              });
              results.push({ regNo, action: 'updated', constituency });
            } else {
              // Create new voter
              ...
              results.push({ regNo, action: 'created', constituency });
            }
          } catch (error) {
